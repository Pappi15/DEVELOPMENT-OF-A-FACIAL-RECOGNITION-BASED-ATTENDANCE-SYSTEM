...
gin = tk.Tk()
gin.geometry("500x300")
# Roll No
roll_text = StringVar()
roll_label = Label(gin, text='Roll NO', font=('bold', 14))
roll_label.grid(row=0, column=2, sticky=W)
roll_entry = Entry(gin, textvariable=roll_text)
roll_entry.grid(row=0, column=3)

b1 = tk.Button(gin, text='Show Details', width=15,bg='green',
...
Name = tk.Label(gin,  textvariable=name_str, width=30)  
Name.grid(row=3, column=2) 
name_str.set(" ")

loc_str = tk.StringVar()
loc = tk.Label(gin,  textvariable=loc_str, width=30)  
loc.grid(row=5, column=2) 
loc_str.set(" ")

time_str = tk.StringVar()
Time = tk.Label(gin,  textvariable=time_str, width=30 )  
